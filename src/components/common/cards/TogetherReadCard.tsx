...
const TogetherReadCard: React.FC<TogetherReadCardProps> = ({
  ...
  return (
    <div className={cn('bg-beige2 w-full rounded-l px-5 py-5 shadow-sm', className)}>
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-[5px]">
          <p className="text-caption4 text-gray3">{totalDays}일 동안 함께 읽는 책</p>
          <h3 className="text-title6 truncate">{title}</h3>

          <p className="text-caption4 text-gray3">
            남은 기간 <span className="text-green1">D-{remainDays}</span> · 참여자{' '}
        ...
        <div className="flex-shrink-0">
          {isJoined ? <CircleProgress percent={animatedProgress} /> : <CircleNotJoined />}
        </div>
      </div>
      ...
      <div className="mt-5 flex justify-center">
        <button
          className="bg-green1 btn-text-white text-caption5 h-7.5 cursor-pointer rounded-2xl px-10"
          onClick={onClick}
        >
    ...
    <div className="flex items-center justify-center">
      ...
      >
        <div className="bg-beige2 flex h-14 w-14 items-center justify-center rounded-full">
          <span className="text-body1 text-green1">{Math.round(clamped)}%</span>
        </div>
      </div>
    </div>
  );
